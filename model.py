...
from config import NUM_SPATIAL_CHANNELS, NUM_GCC_CHANNELS
...
class SEBlock(nn.Module):
    ...
    def __init__(self, channels, reduction=16):
        ...
        self.excitation = nn.Sequential(
            nn.Linear(channels, channels // reduction, bias=False), nn.ReLU(inplace=True), nn.Linear(channels // reduction, channels, bias=False), nn.Sigmoid()
        )
...
class SpatialAudioHeatmapLocator(nn.Module):
    def __init__(self, input_channels=NUM_SPATIAL_CHANNELS, gcc_channels=NUM_GCC_CHANNELS, azi_bins=180):
        ...
        self.init_conv = nn.Sequential(CoordConv2d(input_channels, 64, kernel_size=(5, 5), stride=(2, 1), padding=(2, 2)), nn.BatchNorm2d(64), nn.ReLU(inplace=True))
        ...
        self.rnn = nn.GRU(input_size=rnn_input_dim, hidden_size=1024, num_layers=2, batch_first=True, bidirectional=True, dropout=0.1)

        # 3. ATTENTION POOLING
        self.attn_pool = AttentionPool(1024 * 2)

        # 4. AZIMUTH HEAD
        self.head = nn.Sequential(nn.Linear(1024 * 2, 1024), nn.Dropout(0.3), nn.ReLU(), nn.Linear(1024, azi_bins))
